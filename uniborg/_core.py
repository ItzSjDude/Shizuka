...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"Bhosdike Successfully loaded plugin {shortname}")
        ...
        await event.respond(f"Failed to load pligon {shortname}: {e}")
...
async def send_plug_in(event):
    ...
    await event.edit("Bkl is plugin ko upload kar diya {} in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("oh! Mdc, yo plugin  instull na hove.")
        except Exception as e:  # pylint:disable=C0103,W0703
